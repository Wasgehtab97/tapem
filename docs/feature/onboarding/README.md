...
- Server-side assignment of deterministic member numbers (0001–9999) for each gym.
...
- Implemented via the Cloud Function `assignMemberNumber` (see `functions/onboarding.js`).
- Trigger: `onCreate` of `/gyms/{gymId}/users/{userId}`.
- Logic:
  1. Abort if the membership document already contains a `memberNumber` (idempotent).
  2. Read `/gyms/{gymId}/config/onboarding.nextMemberNumber` inside a transaction.
...
- All writes happen inside a single Firestore transaction to avoid race conditions and to honour security rules (members cannot mutate the config collection).
...
- Functions Jest tests: `functions/__tests__/onboarding.test.js` validates transaction behaviour.
...
## Migration Notes
- Deploy the Cloud Function before enabling the UI to guarantee number assignment for new registrations.
- For legacy data, run an ad-hoc backfill script (e.g., temporary Cloud Function or admin script) that iterates memberships without `memberNumber`, invoking the same transaction logic as the trigger.
